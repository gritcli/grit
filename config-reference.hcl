...
#
# This block is optional, but if it specified it may only be present within one
...
source "example_source" "some_driver" {
    ...
    # The "clones" block configures Grit behaves with working with local clones
    # of repositories that were obtained from this soure.
    #
    # This block is optional, by default Grit basis its behavior of the
    # top-level "git" block.
    clones {
        # The "dir" attribute is the path to a directory into which local clones
        # are placed.
        #
        # By defaults, clones from this source are kept in a sub-directory of
        # the directory configured by the top-level "git" block. The
        # sub-directory is named the same as the source.
        #
        # For example, if the top-level "git" block uses a "dir" of "~/clones",
        # the default for this source would be "~/clones/example_source".
